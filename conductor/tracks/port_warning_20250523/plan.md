...
## Phase 1: Logic and Store Integration
- [ ] Task: Implement `checkPortOwnership` action in `systemStore.ts`.
    - [ ] Action should take a `port: number` and return the project name if found in `projectsStore`.
- [ ] Task: Write unit tests for `systemStore.checkPortOwnership`.
    - [ ] Test case: Port owned by a project (via `ports`).
    - [ ] Test case: Port owned by a project (via `managedPorts`).
    - [ ] Test case: Port not owned.
- [ ] Task: Conductor - User Manual Verification 'Phase 1: Logic and Store Integration' (Protocol in workflow.md)
