    ...
    >
      <q-toolbar class="q-px-md">
        ...
        >
          ...
          <q-badge
            v-if="systemStore.notifications.length"
            floating
            color="negative"
            rounded
            size="xs"
          ...
          >
            ...
            >
              ...
              <q-item
                v-for="notif in systemStore.notifications"
                :key="notif.id"
                class="q-py-sm"
              >
                <q-item-section avatar>
                  <q-avatar
                    :color="notif.color + '-transparent'"
                    :text-color="notif.color"
                    size="40px"
                  >
                    <q-icon
                      :name="'mdi-' + (notif.icon === 'search' ? 'magnify' : (notif.icon === 'history' ? 'history' : 'check'))"
                      size="20px"
                    />
                  </q-avatar>
                </q-item-section>
                <q-item-section>
                  ...
                  >{{ notif.time }}</q-item-label>
                </q-item-section>
              </q-item>

              <q-item
                v-if="!systemStore.notifications.length"
                class="text-center q-pa-md text-wcag-caption italic"
              >
    ...
    >
      <q-list class="q-mt-lg">
        ...
        >
          ...
          <q-item-section>
            <q-item-label
              class="text-weight-bolder tracking-wide"
              style="font-size: 0.9rem"
            >{{ $t(link.title) }}</q-item-label>
          </q-item-section>
        </q-item>
      </q-list>

      <div
        class="absolute-bottom q-pa-md text-center text-wcag-caption opacity-50"
